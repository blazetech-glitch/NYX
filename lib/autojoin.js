/**
 * SUPËR-MD - Auto Join WhatsApp Group Module
...
module.exports = {
    ...
    initAutoJoin: async (conn, logger = console) => {
        try {
            ...
            logger.log('🔄 Attempting to auto-join SUPËR-MD group...');
            ...
            try {
                ...
                logger.log('✅ Successfully joined SUPËR-MD group from GROUP_LINK');
                ...
                try {
                    ...
                    if (groupInfo && groupInfo.id) {
                        await conn.sendMessage(groupInfo.id, {
                            text: `✅ *SUPËR-MD Bot Joined*\n\n👋 Hello! I'm SUPËR-MD Bot\n\n📋 Type .menu to see all available commands\n\n🔗 Group Link Setup Complete!\n\n*Happy using the bot!*`
                        });
                    }
                } catch (msgErr) {
                ...
                if (errorMsg.includes('already')) {
                    logger.log('ℹ️  Already a member of the SUPËR-MD group');
                    return true;
    ...
    /**
     * Verify if bot is in SUPËR-MD group
     * @param {object} conn - WhatsApp connection instance
    ...
    /**
     * Force rejoin SUPËR-MD group (useful for reconnection)
     * @param {object} conn - WhatsApp connection instance
     * @returns {boolean} - Success status
     */
    forceRejoin: async (conn) => {
        console.log('🔄 Force rejoining SUPËR-MD group...');
